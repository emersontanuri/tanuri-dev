...
A Projeto is checked against the collection schema while the page that lists the collection renders, and a document that does not match stops the build. The check lives in `app/utils/assertProjects.ts`, the rules it applies live in `shared/projetos.ts`, and `nitro.prerender.failOnError` is what turns a page that threw into a failed build. The browser is left out of it: the schema is imported behind an `import.meta.server` guard, so a visitor is never sent a validator to re-check content the build already accepted.
